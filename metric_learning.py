# -*- coding: utf-8 -*-
"""
Created on Mon May 23 01:43:46 2022
...
def main_test_all_simulations(n = 1000, d = 10, c = 0.2, no_folder = 500, start_folder = 1):
    path_outer = str('E:\\all_datasets\\simu_n_') + str(n) + str('_d_') + str(d) + str('_C_') + str(c)  + str('\\')
    for folder in np.arange(start_folder,no_folder + 1,1):
        begin = time.time()
        path = path_outer + str(folder)
        
        testing_data = read_csv(path + str('\\met_matched.csv'))
        ...
        clusters_2, centers_2, matrix = cop_kmeans_metric_learning(dataset = data, learn = True, diag = True)       
        
        
        result2 = np.vstack((true_label, clusters_2)).T
        pair_2 = pd.DataFrame(result2)
        pair_2 = pair_2.rename(columns={0: 'true_label', 1: 'predicted_label'})
        pair_2.to_csv(path + str('\\met_with_metric_learning_diag.csv'))
        
        
        
        testing_data = read_csv(path + str('\\opt_matched.csv'))
        testing_data = testing_data.sort_values(by=['matched_set'])
        data = ((testing_data.values)[:,1:(1 + d)]).astype(float)
        true_label = ((testing_data.values)[:,(1 + d)]).astype(float)
        clusters_2, centers_2, matrix = cop_kmeans_metric_learning(dataset = data, learn = True, diag = True)              
        ...
        pair_2 = pd.DataFrame(result2)
        pair_2 = pair_2.rename(columns={0: 'true_label', 1: 'predicted_label'})
        pair_2.to_csv(path + str('\\opt_with_metric_learning_diag.csv'))
        
        
        testing_data = read_csv(path + str('\\psm_matched.csv'))
        testing_data = testing_data.sort_values(by=['matched_set'])
        data = ((testing_data.values)[:,1:(1 + d)]).astype(float)
        true_label = ((testing_data.values)[:,(1 + d)]).astype(float)
        clusters_2, centers_2, matrix = cop_kmeans_metric_learning(dataset = data, learn = True, diag = True)     
        ...
        pair_2 = pd.DataFrame(result2)
        pair_2 = pair_2.rename(columns={0: 'true_label', 1: 'predicted_label'})
        pair_2.to_csv(path + str('\\psm_with_metric_learning_diag.csv'))
        
        print('Current n is', n, 'Current d is', d, 'Current c is', c, 'Current no_folder is', folder)
        print('Time for this folder is', time.time() - begin, 's')
        print('#######################################################################################')     
...
label = ((data.values)[:,11]).astype(float)
n,p = test_data.shape
